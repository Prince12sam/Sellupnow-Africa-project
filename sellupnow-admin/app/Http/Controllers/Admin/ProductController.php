...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Default behavior: show approved/active products unless an explicit request-filter is provided.
        $filterApproved = $approve || (! $request->filled('status') && ! $request->filled('approve'));

        $products = Product::query()
        ->when($status === '1' || $status === 1, function ($query) {
            return $query->where('is_approve', false)->where('is_new', false);
        })->when($status === '0' || $status === 0, function ($query) {
            return $query->where('is_approve', false)->where('is_new', true);
        })->when($filterApproved, function ($query) {

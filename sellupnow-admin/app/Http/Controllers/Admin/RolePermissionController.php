...
class RolePermissionController extends Controller
{
    ...
    public function store(RoleRequest $request)
    {
        if ($this->listOceanAdminAdapter->enabled()) {
            return $this->listOceanAdminAdapter->forward($request, 'POST', '/roles', [
                'name' => $request->name,
                'guard_name' => 'web',
    ...
    {
        if ($this->listOceanAdminAdapter->enabled()) {
            $response = $this->listOceanAdminAdapter->forward($request, 'PUT', '/roles/'.$role->id, [
                'name' => $request->name,
                'is_shop' => $request->for_shop ? true : false,
            ]);
    ...
    public function destroy(Role $role)
    {
        ...
        if ($this->listOceanAdminAdapter->enabled()) {
            $response = $this->listOceanAdminAdapter->forward(request(), 'DELETE', '/roles/'.$role->id);
    ...
    public function rolePermission(Role $role)
    {
        ...
        if ($generaleSetting?->shop_type == 'single') {
            ...
            if ($role->is_shop) {
                // for multi shop role
                $allPermissionArray['shopMultiShop'] = config('acl.permissions.shopMultiShop');
                $allPermissionArray['shop'] = config('acl.permissions.shop');
            } else {
    ...
    {
        if ($this->listOceanAdminAdapter->enabled()) {
            $response = $this->listOceanAdminAdapter->forward($request, 'POST', '/roles/'.$role->id.'/permissions', [

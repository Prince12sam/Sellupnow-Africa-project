...
class ChatController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        // Push notification to the shop owner
        try {
            $shop = Shop::find($request->shop_id);
            if ($shop && $shop->user_id && $shop->user_id !== $auth->id) {
                PushNotificationService::sendToUsers(
                    $shop->user_id,
                    'New message from ' . ($auth->name ?? 'a customer'),
                    $request->type === 'text' ? ($request->message ?? '') : '📎 Attachment',
                    ['type' => 'chat_message', 'shop_id' => (string) $request->shop_id]
                );
            }
        } catch (\Throwable $th) {
            report($th);

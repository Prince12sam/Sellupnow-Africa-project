...
if (! function_exists('listocean_core_path')) {
    /**
     * Return the absolute path to the ListOcean frontend core directory.
     *
     * Configurable via LISTOCEAN_CORE_PATH in .env.
     * Falls back to the conventional sibling-directory layout used in development
     * (admin app sits next to the main-file/listocean/core dir).
     *
     * @param  string  $path  Optional relative sub-path to append (e.g. 'public', '.env').
     */
...
if (! function_exists('generaleSetting')) {
    ...
    function generaleSetting($type = null, $authUser = null)
    {
        ...
        if ($type == 'shop') {
            // Shop panel removed — always return null.
            return null;
        }

...
class ThemeColorRepository extends Repository
{
    ...
    public static function changeStyleCSS($primary ,$secondary){
        ...
        // Keep Listocean frontend theme colors in sync (Sellupnow is the single admin).
        self::syncListoceanThemeColors($primary, $secondary);
    ...
    private static function syncListoceanThemeColors(string $primary, string $secondary): void
    {
        try {
            $db = DB::connection('listocean');
            $pairs = [
                'site_main_color_one' => $primary,
                'site_main_color_two' => $secondary,
                'site_secondary_color' => $secondary,
            ];

            foreach ($pairs as $optionName => $optionValue) {
                $exists = $db->table('static_options')->where('option_name', $optionName)->exists();
                if ($exists) {
                    $db->table('static_options')->where('option_name', $optionName)->update([
            ...
            Log::error('Failed to sync Listocean theme colors: ' . $e->getMessage());
        }
    }
}

...
class GeneraleSettingRepository extends Repository
{
    ...
    public static function updateByRequest(GeneraleSettingRequest $request): void
    {
        ...
        $adminLoginImg = self::loginBackgroundUpdate($request, 'admin_login_background', $generaleSetting?->admin_login_img);
        $shopLoginImg = self::loginBackgroundUpdate($request, 'shop_login_background', $generaleSetting?->shop_login_img);
        ...
        self::query()->updateOrCreate(
            ...
            [
                ...
                'admin_login_img' => $adminLoginImg,
                'shop_login_img' => $shopLoginImg,
            ]
        );

        // Bust the 30-day helper cache so admin reloads always show fresh data.
        Cache::forget('generale_setting');
        Cache::forget('default_currency');
    }

    private static function loginBackgroundUpdate(GeneraleSettingRequest $request, string $inputName, ?string $existingPath): ?string
    {
        // Primary mode: <x-image-picker> stores a string path in a hidden input.
        $raw = trim((string) $request->input($inputName, ''));

        // Optional mode: if later converted to a real file input, support file uploads too.
        if ($request->hasFile($inputName)) {
            ...
            $newPath = Storage::disk('public')->putFile('login-background', $file);
        ...
        return self::normalizePublicDiskPath($raw);
    }

    private static function normalizePublicDiskPath(string $value): string
    {
        $value = trim($value);
        if ($value === '') {
            return $value;
        ...
        // If saved as URL, keep as-is (views will render it directly).
        if (preg_match('/^https?:\/\//i', $value)) {
            return $value;
        }

        // If saved as /storage/foo/bar.jpg or storage/foo/bar.jpg, normalize to public-disk relative path: foo/bar.jpg
        $value = ltrim($value, '/');

    ...
    public static function OrderStatusUpdateFromRider(Order $order, $driverOrder, $orderStatus)
    {
        ...
        if (($orderStatus == 'deliveredAndPaid') || $isDelivery) {
            ...
            if ($generaleSetting?->business_based_on == 'commission' && $generaleSetting?->commission_charge != 'monthly') {
                // Use new CommissionService when enforcement is enabled via env var
                if (env('MONETIZATION_ENFORCE', false)) {
                    try {
                        $calc = CommissionService::calculate((float) $order->total_amount, ['shop_id' => $order->shop_id]);
                        $commission = (float) ($calc['commission'] ?? 0);
                        // record enforced commission audit
                        CommissionService::audit('commission_enforced', $commission, [
                            'order_id' => $order->id,
                            'calc' => $calc,
                        ], null);
                    } catch (Throwable $e) {
                        Log::warning('Commission enforcement failed for order '.$order->id.': '.$e->getMessage());
                    ...
                    // Legacy calculation preserved; also record a shadow audit using CommissionService
                    if ($generaleSetting?->commission_type != 'fixed') {
                        $commission = $order->total_amount * $generaleSetting->commission / 100;
                    } else {
                        $commission = $generaleSetting->commission ?? 0;
                    ...
                    try {
                        $calc = CommissionService::calculate((float) $order->total_amount, ['shop_id' => $order->shop_id]);
                        CommissionService::audit('commission_shadow_on_delivery', (float) ($calc['commission'] ?? 0), [
                            'order_id' => $order->id,
                            'calc' => $calc,
                        ], null);
                    } catch (Throwable $e) {

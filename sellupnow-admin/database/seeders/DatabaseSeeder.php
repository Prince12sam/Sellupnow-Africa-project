...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (app()->environment('local')) {
            $seedCommerceDemos = (bool) env('SEED_COMMERCE_DEMOS', false);
    ...
    private function userInfo()
    {
        ...
        $this->command->warn('- Email: root@sellupnow.com');
        $this->command->warn('- Password: secret');
        $this->command->info('');

        if (app()->environment('local') && (bool) env('SEED_COMMERCE_DEMOS', false)) {
            // info for shop user in command line
            $this->command->info('Demo Shop created:');
            $this->command->warn('- Email: shop@sellupnow.com');
            $this->command->warn('- Password: secret');

            // info for rider user in command line
            $this->command->info('Rider created:');
            $this->command->warn('- Email: rider@sellupnow.com');
            $this->command->warn('- Password: secret');
        }
    }
}

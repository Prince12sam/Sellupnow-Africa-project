...
@section('content')
    <div class="d-flex align-items-center justify-content-between px-3">
        <div><h4 class="mb-0">{{ __('Add Feature') }}</h4></div>
        <div><a href="{{ route('admin.membershipFeature.index') }}" class="btn btn-outline-secondary">{{ __('Back') }}</a></div>
    </div>

    <div class="container-fluid mt-3">
        <div class="card"><div class="card-body">
            <form method="POST" action="{{ route('admin.membershipFeature.store') }}">
                @csrf
                <div class="mb-3"><label class="form-label">Key</label><input name="key" class="form-control" required></div>
                ...
                <div class="form-check"><input type="checkbox" name="is_active" value="1" class="form-check-input" id="is_active" checked><label for="is_active" class="form-check-label">Active</label></div>

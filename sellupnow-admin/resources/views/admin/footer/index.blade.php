    ...
    <div class="container-fluid my-3">
        ...
        <div class="card mb-3">
            <div class="card-header py-2.5">
                <h4 class="card-title m-0 fz-16">{{ __('Customer Web Footer') }}</h4>
                <p class="fs-16 m-0">{{ __('Manage customer web footer (logo + contact + copyright) from here.') }}</p>
            </div>
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-lg-6">
                        <form action="{{ route('admin.footer.website-logos') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="dropzone-container">
                                <label for="ListoceanFooterLogoOnFooter" class="logoRatio">
                                    <img src="{{ $listoceanFooterLogoUrl ?? 'https://placehold.co/200x50/png' }}" id="previewListoceanFooterLogoOnFooter" alt="" width="100%" class="dropzone-area">
                                </label>
                                <div class="mt-2">
                                    <h5 class="mb-1">{{ __('Footer Logo (Customer Web)') }}</h5>
                                    @error('listocean_footer_logo')
                                        <p class="text-danger mb-0">{{ $message }}</p>
                                    @enderror
                                </div>
                                <input id="ListoceanFooterLogoOnFooter" accept="image/*" type="file" name="listocean_footer_logo" class="form-control" onchange="previewFile(event, 'previewListoceanFooterLogoOnFooter')">
                            </div>

                            <div class="d-flex justify-content-end mt-3">
                                <button type="submit" class="btn btn-primary btn-sm">{{ __('Update Customer Web Footer Logo') }}</button>
                            </div>
                        </form>
                    </div>

                    <div class="col-lg-6">
                        <form action="{{ route('admin.footer.website-content') }}" method="POST">
                            @csrf

                            @error('listocean_footer')
                                <div class="alert alert-danger py-2">{{ $message }}</div>
                            @enderror

                            <div class="mb-2">
                                <label class="form-label mb-1">{{ __('Address (Customer Web)') }}</label>
                                <input type="text" name="listocean_address" class="form-control" value="{{ $listoceanFooterContact['address'] ?? '' }}">
                            </div>
                            <div class="mb-2">
                                <label class="form-label mb-1">{{ __('Phone (Customer Web)') }}</label>
                                <input type="text" name="listocean_phone" class="form-control" value="{{ $listoceanFooterContact['phone'] ?? '' }}">
                            </div>
                            <div class="mb-2">
                                <label class="form-label mb-1">{{ __('Email (Customer Web)') }}</label>
                                <input type="email" name="listocean_email" class="form-control" value="{{ $listoceanFooterContact['email'] ?? '' }}">
                            </div>
                            <div class="mb-2">
                                <label class="form-label mb-1">{{ __('Copyright (Customer Web)') }}</label>
                                <input type="text" name="listocean_copyright" class="form-control" value="{{ $listoceanCopyrightText ?? '' }}">
                                <small class="text-muted">{{ __('Tip: use {copy} and {year} placeholders.') }}</small>
                            </div>

                            <div class="d-flex justify-content-end mt-3">
                                <button type="submit" class="btn btn-primary btn-sm">{{ __('Update Customer Web Footer Content') }}</button>
                            </div>
                        </form>
                    </div>
                </div>

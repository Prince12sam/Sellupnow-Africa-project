...
@push('scripts')
    ...
    <script>
        var _newNodeKey = @json(session('newNodeKey'));
        ...
        $('#categoryTree').on('ready.jstree', function() {
            ...
            // If a node was just created, open all ancestors, select and scroll to it
            if (_newNodeKey) {
                var tree = $('#categoryTree').jstree(true);
                // Open all so new nodes (especially sub/child) become visible
                tree.open_all();
                // Wait for open_all to finish then select & scroll
                setTimeout(function() {
                    tree.deselect_all();
                    tree.select_node(_newNodeKey);
                    var $node = $('#' + _newNodeKey);
                    if ($node.length) {
                        var offsetTop = $node.offset().top - 100;

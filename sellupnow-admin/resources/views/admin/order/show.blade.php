...
@section('content')
                ...
                <div class="card-header d-flex align-items-center justify-content-between gap-2 py-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        @hasPermission(['shop.order.attach.barcode'])
                            @if (module_exists('purchase') && \Illuminate\Support\Facades\Route::has('shop.order.attach.barcode'))
                            ...
                            @endif
                        @endhasPermission
                        @if (\Illuminate\Support\Facades\Route::has('admin.order.payment-slip'))
                            <a href="{{ route('admin.order.payment-slip', $order->id) }}" target="_blank"
                        ...
                        @endif
                        @if (\Illuminate\Support\Facades\Route::has('admin.order.download-invoice'))
                            <a href="{{ route('admin.order.download-invoice', $order->id) }}" target="_blank"
    ...
    @if (module_exists('purchase') && (\Illuminate\Support\Facades\Route::has('admin.order.attach.barcode') || \Illuminate\Support\Facades\Route::has('shop.order.attach.barcode')))
        <form id="scannerForm" method="POST"
            action="{{ \Illuminate\Support\Facades\Route::has('admin.order.attach.barcode') ? route('admin.order.attach.barcode') : route('shop.order.attach.barcode') }}">
            @csrf
    ...
    @push('scripts')
        ...
        <script>
            ...
            function fetchProductsBySku(sku) {
                @if (!\Illuminate\Support\Facades\Route::has('admin.order.fetch.products') && !\Illuminate\Support\Facades\Route::has('shop.order.fetch.products'))
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: "{{ __('Barcode fetch route is unavailable') }}"
                });
                return;
                @endif

                $.ajax({
                    url: "{{ \Illuminate\Support\Facades\Route::has('admin.order.fetch.products') ? route('admin.order.fetch.products') : route('shop.order.fetch.products') }}",
                    type: "post",
                    data: {
                        sku: sku,
                        _token: "{{ csrf_token() }}",

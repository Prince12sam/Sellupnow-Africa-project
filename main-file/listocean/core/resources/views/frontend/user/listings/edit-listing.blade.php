                                ...
                                <div class="row g-4">
                                        ...
                                        <div class="post-add-wraper">
                                            <div class="item-name box-shadow1 p-24">
                                                <div class="d-flex align-items-center justify-content-between mb-2">
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-outline-primary d-flex align-items-center gap-1" id="aiSuggestBtn" data-bs-toggle="modal" data-bs-target="#aiListingModal" title="{{ __('Let AI help you improve this listing') }}">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="right-sidebar">
                                                ...
                                                <div class="price-wraper">
                                                    ...
                                                    <div class="negotiable">
                                                        ...
                                                        <label class="mt-2 d-block">
                                                            <input type="checkbox" class="custom-check-box" name="escrow_enabled" id="escrow_enabled" value="1" @if(!empty($listing->escrow_enabled)) checked @endif>
                                            ...
                                            {{-- Video upload (members only, quota-gated) --}}
                                            @if($canUploadVideo ?? false)
                                            @php $adVideo = \App\Models\Backend\AdVideo::where('listing_id', $listing->id)->latest()->first(); @endphp
                                            <div class="item-name box-shadow1 p-24 mt-4">
                                                ...
                                                {{-- Show status of existing AdVideo if present --}}
                                                @if($adVideo)
                                                    @if($adVideo->is_approved)
                                                        <div class="alert alert-success small mb-2">✅ {{ __('Your video is approved and live in the Reels feed.') }}</div>
                                                    @elseif($adVideo->is_rejected)
                                                        <div class="alert alert-danger small mb-2">❌ {{ __('Your video was rejected.') }}{{ $adVideo->reject_reason ? ' '.__("Reason:").' '.$adVideo->reject_reason : '' }}
                                                            <br><em class="text-muted">{{ __('Upload a new video below to replace it.') }}</em>
                                                        </div>
                                                    @else
                                                        <div class="alert alert-warning small mb-2">⏳ {{ __('Your current video is pending admin review.') }}</div>
                                                    @endif
                                                @endif
                                                <div class="mt-3">
                                                    <label class="form-label">{{ $adVideo ? __('Replace Video') : __('Video File') }}</label>
    ...
    (function () {
        ...
        if (aiGenerateBtn) {
            aiGenerateBtn.addEventListener('click', function () {
                ...
                if (!keywords) { alert('{{ __("Please describe what you are selling.") }}'); return; }
        ...
        if (applyDescBtn) {
            applyDescBtn.addEventListener('click', function () {
                ...
                if (descEl) {
                    if (typeof $ !== 'undefined' && $(descEl).hasClass('summernote')) {
        ...
        if (applyBothBtn) {
            applyBothBtn.addEventListener('click', function () {
                var a = document.getElementById('aiApplyTitle');
                var b = document.getElementById('aiApplyDesc');
                if (a) a.click();
                if (b) b.click();

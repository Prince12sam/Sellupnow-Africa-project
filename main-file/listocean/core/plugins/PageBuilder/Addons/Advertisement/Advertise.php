...
use LanguageFallbackForPageBuilder;
    ...
    public function admin_render()
    {
        ...
        $output .= Text::get([
            'name' => 'ad_slot',
            'label' => __('Ad Slot (optional — target a specific slot, e.g. sellupnow:homepage_after_hero)'),
            'value' => $widget_saved_values['ad_slot'] ?? null,
        ]);
    ...
    public function frontend_render()
    {
        ...
        $ad_slot = SanitizeInput::esc_html($this->setting_item('ad_slot'));

        $add_query = Advertisement::query();
        // If a specific slot is configured, use it directly (bypasses type/size filters)
        if (!empty($ad_slot)) {
            $add = $add_query->where('slot', $ad_slot)->where('status', 1)->inRandomOrder()->first();
        } else {

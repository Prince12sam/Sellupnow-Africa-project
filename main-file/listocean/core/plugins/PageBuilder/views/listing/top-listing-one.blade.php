...
<section class="featureListing" data-padding-top="{{$padding_top}}" data-padding-bottom="{{$padding_bottom}}" style="background-color:{{$section_bg}};margin-top:60px;">
    @php
        $normalizedTitle = trim((string) ($section_title ?? ''));
        $isLegacyTop = in_array(strtolower($normalizedTitle), ['top listing', 'top listings'], true);
        $displayTitle = ($normalizedTitle === '' || $isLegacyTop) ? 'Featured Listing' : $normalizedTitle;
    @endphp
    <div class="container-1440">
        <div class="titleWithBtn d-flex justify-content-between align-items-center mb-40">
            <h2 class="head3">{{ __($displayTitle) }}</h2>

...
function load_google_fonts()
{
    ...
    $body_font_variant_selected_arr = !empty($body_font_variant) ? unserialize($body_font_variant, ['allowed_classes' => false]) : ['400'];
    ...
    $heading_font_variant_selected_arr = !empty($heading_font_variant) ? unserialize($heading_font_variant, ['allowed_classes' => false]) : ['400'];
...
function get_attachment_image_by_id($id, $size = null, $default = false)
{
    ...
    $resolveImageUrl = function (array $relativePaths) {
    ...
    $basePath = optional($image_details)->path;

    if (!empty($basePath)) {
        $image_url = $resolveImageUrl([
            'assets/uploads/media-uploader/' . $basePath,
        ]);
    }
    ...
    if (!empty($id) && !empty($image_details)) {
        switch ($size) {
            case "large":
                $image_url = $resolveImageUrl([
                    'assets/uploads/media-uploader/large/large-' . $image_details->path,
                    'assets/uploads/media-uploader/large-' . $image_details->path,
                    'assets/uploads/media-uploader/' . $image_details->path,
                ]);
                break;
            case "grid":
                $image_url = $resolveImageUrl([
                    'assets/uploads/media-uploader/grid/grid-' . $image_details->path,
                    'assets/uploads/media-uploader/grid-' . $image_details->path,
                    'assets/uploads/media-uploader/' . $image_details->path,
                ]);
                break;
            ...
            case "semi-large":
                $image_url = $resolveImageUrl([
                    'assets/uploads/media-uploader/semi-large/semi-large-' . $image_details->path,
                    'assets/uploads/media-uploader/semi-large-' . $image_details->path,
                    'assets/uploads/media-uploader/' . $image_details->path,
                ]);
                break;
            case "thumb":
                $image_url = $resolveImageUrl([
                    'assets/uploads/media-uploader/thumb/thumb-' . $image_details->path,
                    'assets/uploads/media-uploader/thumb-' . $image_details->path,
                    'assets/uploads/media-uploader/' . $image_details->path,
                ]);
                break;
            default:
...
function get_user_lang_direction()
{
    try{
        ...
        if(!empty(session()->get('lang'))){
            return optional($user_direction)->direction ?? optional($default)->direction ?? 'ltr';
        }
        return optional($default)->direction ?? 'ltr';
    }catch(\Exception $e){
        return 'ltr';
    }
}
...
function admin_default_lang(){
    try{
        ...
        return optional($default_lang)->slug ?? 'en';
    }catch (\Exception $e){
        return 'en';
    }
}
...
function get_default_language_direction(){
    try{
        ...
        return optional($default_lang)->direction ?? 'ltr';
    }catch (\Exception $e){
        return 'ltr';
    }
}
...
function render_page_meta_data_for_listing($listing_details){
    ...
    if (!empty($listing_details->condition) && strtolower($listing_details->condition) === 'new') {
        $itemCondition = 'https://schema.org/NewCondition';
    }
    $jsonTitle       = addslashes(strip_tags($listing_details->title ?? ''));
    $jsonDescription = addslashes(strip_tags($site_description ?: ($listing_details->description ?? '')));
    $jsonImage       = $facebook_meta_image;
    $jsonPrice       = is_numeric($listing_details->price ?? null) ? number_format((float)$listing_details->price, 2, '.', '') : '';
    $jsonCurrency    = strtoupper(config('settings.site_currency_sign', 'USD'));
    $priceBlock      = $jsonPrice ? '"offers":{"@type":"Offer","price":"{$jsonPrice}","priceCurrency":"{$jsonCurrency}","itemCondition":"{$itemCondition}","url":"{$site_url}"},' : '';
